...
export const PricingCard = ({ name, price, calls, languages, popular }: PricingCardProps) => {
  return (
    <div className={`card relative hover-lift ${popular ? "border-red-500 ring-2 ring-red-500/20 shadow-lg" : ""}`}>
      {popular && (
        <div className="absolute -top-3 left-1/2 -translate-x-1/2">
          <span className="badge bg-red-500 text-white border-red-500 shadow-lg px-4 py-1 font-semibold">
            Le plus populaire
          </span>
        </div>
      )}
      ...
      <div className="mb-6 pt-2">
        ...
        <div className="flex items-baseline gap-2">
          <span className={`text-5xl font-bold ${popular ? "text-red-500" : "text-blue-500"}`}>
            {price}€
          </span>
      ...
      <ul className="mb-6 space-y-3 py-4 border-t border-border">
        <li className="flex items-start gap-3">
          <div className={`flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center mt-0.5 ${
            popular ? "bg-red-100" : "bg-blue-100"
          }`}>
            <Check className={`h-3 w-3 ${popular ? "text-red-500" : "text-blue-500"}`} />
          </div>
      ...
      <Link
        ...
        className={`block text-center font-semibold ${
          popular 
            ? "btn bg-red-500 hover:bg-red-600 text-white border-red-500 w-full" 
            : "btn-outline w-full"
        }`}
      >
        Choisir {name}
